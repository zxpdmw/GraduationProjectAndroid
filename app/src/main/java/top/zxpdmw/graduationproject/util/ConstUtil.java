package top.zxpdmw.graduationproject.util;

public class ConstUtil {
    public static final String SYSTEM_EXCEPTION="系统异常,请稍后充重试!";
    public static final String TO_LOGIN_ACTIVITY = "准备跳转登录页面";

    public static final String QUERY_BY_ID="?id=";
    public static final String QUERY_BY_USERNAME="?username=";
    public static final String BASE_URL = "http://39.96.113.190:8080/";
    //管理员模块
    public static final String ADMIN_NOTICE="admin/notice";
    public static final String ADMIN_HOUSE_KEEPING="admin/housekeeping";
    public static final String ADMIN_COMPLAIN_REPAIR="admin/complainrepair";
    public static final String ADMIN_EDIT_HK="admin/edithk";
    public static final String ADMIN_EDIT_CR="admin/editcr";
    public static final String ADMIN_LOGIN="admin/login";
    //投诉保修模块
    public static final String COMPLAIN_REPAIR_GET="complainrepair/get";
    public static final String COMPLAIN_REPAIR_ADD="complainrepair/add";
    //家政服务模块
    public static final String HOUSE_KEEPING_ADD="housekeeping/add";
    public static final String HOUSE_KEEPING_DELETE="housekeeping/delete";
    public static final String HOUSE_KEEPING_GET_BY_USERNAME="housekeeping/get";
    //房屋租售模块
    public static final String HOUSE_RENT_SALE="houserentsale/all";
    public static final String HOUSE_RENT="houserentsale/rent";
    public static final String HOUSE_SALE="houserentsale/sale";
    public static final String HOUSE_RENT_SALE_PUBLISH="houserentsale/publishe";
    public static final String HOUSE_RENT_SALE_DELETE="houserentsale/delete";
    public static final String HOUSE_RENT_SALE_EDIT_HOUSE="houserentsale/edit";
    //社区公告模块
    public static final String NOTICE_RECOMMEND="notice/recommend";
    public static final String NOTICE_DETAIL="notice/detail";
    public static final String NOTICE_DELETE="notice/delete";
    public static final String NOTICE_PUBLISH="notice/publish";
    //物业费模块
    public static final String PROPERTY_GET="property/get";
    public static final String PROPERTY_ADD="property/pay";
    //用户模块
    public static final String USER_LOGIN="user/login";
    public static final String USER_REGISTER="user/register";
    public static final String USER_PASSWORD="user/password";
    public static final String USER_INFO="user/info";
    public static final String USER_GET_INFO="user/get";
    public static final String USER_GET_HOUSEID="user/houseid";





}
